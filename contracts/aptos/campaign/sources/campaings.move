module campaign_blockchain::campaigns_v3 {
    use std::signer;
    ...
    public entry fun initialize(account: &signer) {
        ...
        assert!(!exists<CampaignStore>(account_addr), 0xA); // Already initialized for this user
    ...
    public entry fun donate_to_campaign(
        ...
        let new_balance =
            if (table::contains(&campaign_store.campaign_balances, campaign_id)) {
                *table::borrow(&campaign_store.campaign_balances, campaign_id) + amount
            } else { amount };

        if (table::contains(&campaign_store.campaign_balances, campaign_id)) {
            table::remove(&mut campaign_store.campaign_balances, campaign_id);
        };
        table::add(&mut campaign_store.campaign_balances, campaign_id, new_balance);
    ...
    public entry fun purchase_business_service(
        ...
        let new_balance =
            if (table::contains(&campaign_store.campaign_balances, campaign_id)) {
                *table::borrow(&campaign_store.campaign_balances, campaign_id) + total_amount
            } else {
                total_amount
            };
